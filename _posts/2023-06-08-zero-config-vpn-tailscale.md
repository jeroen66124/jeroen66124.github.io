...
> This guide is written with the use of Photon OS in mind, but can also be applied to Docker Desktop, Ubuntu Server and all sorts of other docker-compose instances. If you want to know how to set up Tailscale, start the guide from the [Set up Tailscale](https://vskills.nl/posts/run-full-bitcoin-node-self-hosting-umbrel-os/#step-2-set-up-ubuntu-server) section.
{: .prompt-tip }
